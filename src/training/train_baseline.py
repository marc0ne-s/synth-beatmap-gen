"""
Training script for baseline beatmap model.

Designed for overfit testing on a small subset of the corpus.
"""
...
from src.models.baseline import BaselineBeatmapModel, compute_loss, compute_metrics
...
def train_epoch(
    ...
    for batch in dataloader:
        ...
        # Flatten spatial dimensions for target
        # occ: (B, T, 2, X, Y) -> we need presence per hand
        target_presence = pres  # (B, T, 2)
        target_positions = pos  # (B, T, 2, 2) -> flatten to (B, T, 4)
        target_positions = target_positions.view(target_positions.shape[0], target_positions.shape[1], -1)
        target_rails = torch.zeros_like(pres[:, :, 0])  # Placeholder, not in dataset yet

        # Compute per-hand positive weight for class imbalance (~5% positive)
...
def train(
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(
            ...
            f"precision={val_metrics.get('precision', 0):.3f} | "
    ...
    print(f"  F1: {val_metrics.get('f1', 0):.3f}")
...
def main():
    parser = argparse.ArgumentParser(description="Train baseline beatmap model")
