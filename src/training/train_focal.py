#!/usr/bin/env python3
"""
Training script with focal loss for beatmap presence prediction.

Uses the small BaselineBeatmapModel (1.1M params) for fast iteration.
Switches from weighted BCE to focal loss to handle extreme class imbalance.
Saves checkpoints by F1 score, not loss.
"""
...
from src.models.focal_loss import compute_focal_loss
...
def train_epoch(
    ...
    for batch in dataloader:
        ...
        loss, loss_dict = compute_focal_loss(
            predictions,
            target_presence,
            target_positions,
            target_rails,
...
def train(
    ...
    for epoch in range(1, epochs + 1):
        start = time.time()
        train_metrics = train_epoch(model, train_loader, optimizer, device, alpha=alpha, gamma=gamma)
        ...
        if val_metrics["f1"] > best_f1:
            ...
            ckpt_path = os.path.join(output_dir, "focal_best_model.pt")
            torch.save({
                "epoch": epoch,
    ...
    # Save history
    history_path = os.path.join(output_dir, "focal_history.json")
...
def main():
    parser = argparse.ArgumentParser(description="Train beatmap model with focal loss")
    ...
    parser.add_argument("--alpha", type=float, default=0.25, help="Focal loss alpha (positive weight)")
